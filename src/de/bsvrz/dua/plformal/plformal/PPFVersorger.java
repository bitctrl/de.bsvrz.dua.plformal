...
public final class PPFVersorger implements IPPFVersorger,
	 ...
	 */
	private static SystemObjectType typGanzZahl = null;

	/**
	 * Systemobjekt einer Gleitkommazahl.
	 */
	private static SystemObjectType typKommaZahl = null;
	 ...
	 */
	private Collection<IPPFVersorgerListener> listenerListe = new HashSet<IPPFVersorgerListener>();
	...
	private PPFVersorger(final IVerwaltungMitGuete verwaltung,
		...
		typGanzZahl = verwaltung.getVerbindung().getDataModel().getType(
				"typ.ganzzahlAttributTyp"); //$NON-NLS-1$
		typKommaZahl = verwaltung.getVerbindung().getDataModel().getType(
				"typ.kommazahlAttributTyp"); //$NON-NLS-1$
		...
		if (instanz == null) {
			...
			if (plausibilisierungsObjekte.length > 0) {
				...
				throw new DUAInitialisierungsException(
						"Es liegt kein Objekt vom Typ " + //$NON-NLS-1$
								PPFKonstanten.TYP + " vor"); //$NON-NLS-1$
			}
		}
	...
	public SystemObject[] getBetrachteteObjekte() {
		...
		if (this.plBeschreibungen != null) {
			for (DAVObjektAnmeldung anmeldung : this.plBeschreibungen
	...
	public Data plausibilisiere(final ResultData resultat) {
		...
		if (plBeschreibungen == null) {
			Debug.getLogger().fine("Es wurden noch keine Parameter" + //$NON-NLS-1$
				...
				&& resultat.getData() != null) {
			Collection<PPFAttributSpezifikation> attSpezifikationen = this.plBeschreibungen
	...
	private Data plausibilisiereAttribut(final Data datum,
		...
		if (attSpez != null) {
			...
			if (attPfad != null && methode != null) {
				...
				Data plausDatum = DUAUtensilien
						.getAttributDatum(attPfad, dummy);
				if (plausDatum != null
						&& (plausDatum.getAttributeType().isOfType(typGanzZahl) || plausDatum
								.getAttributeType().isOfType(typKommaZahl))
						&& !plausDatum.asUnscaledValue().isState()) {

					double alterWert = plausDatum.asUnscaledValue()
							.doubleValue();
	...
	private void setStatusPlFormalWert(final Data datum, final boolean wert,
		...
		if (datum != null && attPfad != null && attPfadErsetzung != null) {
			...
			if (neuerAttPfad != null) {
				Data status = DUAUtensilien.getAttributDatum(neuerAttPfad,
						datum);
				if (status != null) {
