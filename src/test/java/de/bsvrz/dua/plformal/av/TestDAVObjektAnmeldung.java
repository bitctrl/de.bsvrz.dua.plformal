...
public class TestDAVObjektAnmeldung {
	...
	public void testVerhaltenInTreeSet() {
		...
		final SystemObject[] o = new SystemObject[] {
				TestDAVObjektAnmeldung.verbindung.getDataModel().getObject(
						"mq.a100.0001"),
						TestDAVObjektAnmeldung.verbindung.getDataModel().getObject(
								"mq.a100.0002"),
								TestDAVObjektAnmeldung.verbindung.getDataModel().getObject(
		...
		d[0] = new DataDescription(TestDAVObjektAnmeldung.verbindung
				.getDataModel()
				.getAttributeGroup("atg.verkehrsDatenKurzZeitMq"),
				TestDAVObjektAnmeldung.verbindung.getDataModel().getAspect(
						"asp.agregation1Minute"));

		d[1] = new DataDescription(TestDAVObjektAnmeldung.verbindung
				.getDataModel().getAttributeGroup(
						"atg.verkehrsDatenKurzZeitAnalyseMq"),
						TestDAVObjektAnmeldung.verbindung.getDataModel().getAspect(
								"asp.parameterVorgabe"));

		d[2] = new DataDescription(TestDAVObjektAnmeldung.verbindung
				.getDataModel()
				.getAttributeGroup("atg.verkehrsDatenKurzZeitMq"),
				TestDAVObjektAnmeldung.verbindung.getDataModel().getAspect(
						"asp.agregation5Minuten"));

		d[3] = new DataDescription(TestDAVObjektAnmeldung.verbindung
				.getDataModel()
				.getAttributeGroup("atg.verkehrsDatenKurzZeitMq"),
				TestDAVObjektAnmeldung.verbindung.getDataModel().getAspect(
						"asp.agregation15Minuten"));

		d[4] = new DataDescription(TestDAVObjektAnmeldung.verbindung
				.getDataModel().getAttributeGroup(
						"atg.verkehrsDatenKurzZeitAnalyseMq"),
						TestDAVObjektAnmeldung.verbindung.getDataModel().getAspect(
								"asp.parameterSoll"));

		d[5] = new DataDescription(TestDAVObjektAnmeldung.verbindung
				.getDataModel().getAttributeGroup(
						"atg.verkehrsDatenKurzZeitAnalyseMq"),
						TestDAVObjektAnmeldung.verbindung.getDataModel().getAspect(
								"asp.parameterIst"));
		...
		try {
			...
			final DAVObjektAnmeldung[] doppel = {
					new DAVObjektAnmeldung(o[0], d[0]),
					new DAVObjektAnmeldung(o[1], d[0]),
					new DAVObjektAnmeldung(o[2], d[0]),
					new DAVObjektAnmeldung(o[3], d[0]),
			...
			for (int i = 0; i < TestDAVObjektAnmeldung.GENAUIGKEIT; i++) {
				...
				Assert.assertTrue("Größe der Menge: " + testSet.size() + ", "
			...
			final DAVObjektAnmeldung[] doppel1 = {
					new DAVObjektAnmeldung(o[0], d[0]),
					new DAVObjektAnmeldung(o[1], d[0]),
					new DAVObjektAnmeldung(o[2], d[0]),
					new DAVObjektAnmeldung(o[3], d[0]),
			...
			for (int i = 0; i < TestDAVObjektAnmeldung.GENAUIGKEIT; i++) {
				...
				Assert.assertTrue(
						"Größe der Menge: " + testSet.size() + ", 14",
						testSet.size() == 14);
			...
			final DAVObjektAnmeldung[] doppel2 = {
					new DAVObjektAnmeldung(o[0], d[1]),
					new DAVObjektAnmeldung(o[0], d[0]),
					new DAVObjektAnmeldung(o[0], d[2]),
					new DAVObjektAnmeldung(o[1], d[3]),
			...
			for (int i = 0; i < TestDAVObjektAnmeldung.GENAUIGKEIT; i++) {
				...
				Assert.assertTrue(
						"Größe der Menge: " + testSet.size() + ", 16",
						testSet.size() == 16);
